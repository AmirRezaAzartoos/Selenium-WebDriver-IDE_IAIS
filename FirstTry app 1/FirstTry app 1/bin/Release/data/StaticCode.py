...
#bodyCode#
    ...
    winsound.Beep(freqSuccess, durationSuccess)
    sys.stdout = open("C:\\run-test-selenium\\Source\\Log\\testSuit(Passed-at-" + str(now.strftime("%Y-%m-%d-&-%H-%M-%S")) + ").log", "w", encoding="utf-8")
    print(Info)
    print(TestCases)
    ...
    log.info(StoreEvalDB.vars)
    winsound.Beep(freqError, durationError)
    log.error(traceback.format_exc())
    original_size = driver.get_window_size()
    required_width = driver.execute_script('return document.body.parentNode.scrollWidth')
    required_height = driver.execute_script('return document.body.parentNode.scrollHeight')
    driver.set_window_size(required_width, required_height)
    driver.find_element_by_tag_name('body').screenshot("C:\\run-test-selenium\\Source\\Log\\testSuit(Failed-at-" + str(now.strftime("%Y-%m-%d-&-%H-%M-%S")) + ").png")
    sys.stdout = open("C:\\run-test-selenium\\Source\\Log\\testSuit(Failed-at-" + str(now.strftime("%Y-%m-%d-&-%H-%M-%S")) + ").log", "w", encoding="utf-8")
    print(Info)
    print(TestCases)
